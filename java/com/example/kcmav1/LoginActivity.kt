...
import com.example.kcmav1.utils.showDialog
...
signibtn=findViewById(R.id.sign_in)
       signibtn.setOnClickListener(View.OnClickListener {
            ValidateData()
        })
                ...
                if(pass==password.text.toString()){
                    ...
                    db.collection("users").document(email.text.toString()).get().addOnSuccessListener { documents ->
                        if (documents.exists()) {
                            ...
                            AppPreferences.email=em.toString()
