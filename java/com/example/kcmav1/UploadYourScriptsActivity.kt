...
class UploadYourScriptsActivity : AppCompatActivity(),AdapterView.OnItemSelectedListener {
   ...
   lateinit var  progrss:ProgressDialog
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        select_pdf.setOnClickListener(View.OnClickListener {
            val intent= Intent()
        ...
        upload_song.setOnClickListener(View.OnClickListener {
            upload()
        })
    }
...
displayName=file.name
   ...
   fun upload(){
    ...
    private fun saveSongData(songUrl: String) {
        ...
        val song = UploadSongData(song_title.text.toString().toLowerCase(), user,songUrl, song_composer.text.toString(), song_choir.text.toString(),selectedDio,0,0)

        db.collection("files").document(selectedDio).collection("songs").document(song_title.text.toString()).set(song)
